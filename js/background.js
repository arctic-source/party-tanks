...
import { randInt, lerpColor, hexToRgb, pseudoRandom } from "./utils.js";
...
// A dense city silhouette drawn from `buildings` (store.bgSkylineSets[i],
// generated by scenery.js: generateBgSkyline - a map can use this shape
// twice, at different densities/colors/parallax, for a genuine far/near
// city depth effect rather than one recolored copy). Windows are drawn in
// a second pass, positioned deterministically via pseudoRandom seeded off
// each building's own index so they don't re-randomize (flicker) every
// frame - only the building's own `windows` count (set once at
// generation) decides how many a given building gets.
function drawSkylineLayer(parallaxFactor, alpha, color, baseY, windowColor, buildings) {
  if (!buildings) return;
  var parallax = store.camCenterX * parallaxFactor;
  ...
  for (var i = 0; i < buildings.length; i++) {
    var b = buildings[i];
    var sx = b.rx - parallax;
    if (sx + b.w / 2 < 0 || sx - b.w / 2 > store.VIEW_W) continue;
    ctx.fillRect(sx - b.w / 2, baseY - b.h, b.w, b.h);
  }
  ctx.restore();
  ...
  for (var j = 0; j < buildings.length; j++) {
    var b2 = buildings[j];
    var sx2 = b2.rx - parallax;
    if (sx2 + b2.w / 2 < 0 || sx2 - b2.w / 2 > store.VIEW_W) continue;
    var topY = baseY - b2.h;
    for (var k = 0; k < b2.windows; k++) {
      var wx = pseudoRandom(j * 31 + k * 7 + 1) * (b2.w - 8) + 4;
      var wy = pseudoRandom(j * 17 + k * 13 + 1) * (b2.h - 10) + 5;
      ctx.fillRect(sx2 - b2.w / 2 + wx, topY + wy, 3, 4);
    }
  }
  ctx.restore();
}

// A chain-link-fence-like silhouette right at the terrain line - evenly
// spaced vertical posts with a sagging wire between each pair. Fully
// procedural from the parallax offset + a fixed post spacing (no
// generated array needed, unlike the other shapes above) since the
// pattern is regular, not naturally varied.
function drawFenceLayer(parallaxFactor, alpha, color, baseY) {
  var postSpacing = 70, postHeight = 26, sag = 6;
  var parallax = store.camCenterX * parallaxFactor;
  var offset = parallax % postSpacing;
  ...
  ctx.beginPath();
  var prevX = null;
  for (var x = -offset - postSpacing; x <= store.VIEW_W + postSpacing; x += postSpacing) {
    var topY = baseY - postHeight;
    ctx.moveTo(x, baseY);
    ctx.lineTo(x, topY);
    if (prevX !== null) {
      var midX = (prevX + x) / 2;
      ctx.moveTo(prevX, topY);
      ctx.quadraticCurveTo(midX, topY + sag, x, topY);
    }
    prevX = x;
  }
  ctx.stroke();
...
export function drawBackground() {
  ...
  map.bgLayers.forEach(function (layer, i) {
    ...
    } else if (layer.shape === "skyline") {
      drawSkylineLayer(layer.parallax, layer.alpha, layer.color, baseY, layer.windowColor, store.bgSkylineSets[i]);
    } else if (layer.shape === "fence") {
      drawFenceLayer(layer.parallax, layer.alpha, layer.color, baseY);
    } else {
